...
export default function Leaderboard({ darkMode }) {
  ...
  if (loading) return (
    <div className={`lb-page${darkMode ? " dark" : ""}`}>
  ...
  return (
    <div className={`lb-page${darkMode ? " dark" : ""}`}>
      <div className="lb-header">
        ...
        <select className="lb-select" value={city} onChange={e => setCity(e.target.value)}>
      ...
      {entries.length === 0 && (
        <div style={{ textAlign: "center", color: "#999", padding: "2rem", fontSize: "0.9rem" }}>
          No entries found. Start logging to earn eco-points!
        </div>
      )}
    </div>
  );
