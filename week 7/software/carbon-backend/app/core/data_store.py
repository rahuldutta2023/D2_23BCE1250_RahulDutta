...
class DataStore:
    ...
    def _load_all(self):
        ...
        self.user_goals   = self._load_or_create("user_goals.csv",
            ["goal_id","user_id","monthly_budget_kg","created_at","updated_at"])
        self.eco_actions  = self._load_or_create("eco_actions.csv",
            ["action_id","title","points","category","description"])
        self.user_actions = self._load_or_create("user_actions.csv",
            ["id","user_id","action_id","completed_at"])

        # New tables
        self.user_badges  = self._load_or_create("user_badges.csv",
            ["id","user_id","badge_id","awarded_at"])
        self.notifications = self._load_or_create("notifications.csv",
            ["notification_id","user_id","type","title","message","severity","is_read","created_at"])
    ...
    def save_user_badges(self):        self._save(self.user_badges,       "user_badges.csv")
    def save_notifications(self):      self._save(self.notifications,     "notifications.csv")
